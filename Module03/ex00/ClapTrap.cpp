...
{
    if (!this->hp)
        std::cout << this->name << " has no HitPoints left and can not attack." << std::endl;
    else if (!this->ep)
        std::cout << this->name << " has no EnergyPoints left and can not attack." << std::endl;
    else
    {
        std::cout << this->name << " attacks " << target << ", causing " << this->ap << " points of damage!" << std::endl;
        this->ep--;
    }
    return ;
}
